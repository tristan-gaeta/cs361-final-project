...
class GameObjects {
  ...
  static projectile(x, y) {
    let txt = Matter.Common.choose([
      "images/balls/Color Ball.png",
      "images/balls/Gradient-ball.png",
      "images/balls/8-Ball.png",
    ...
    let body = Matter.Bodies.circle(x, y, GameObjects.BLOCK_SIZE / 2, {
      render: {
        sprite: {
          texture: txt,
          xScale: GameObjects.BLOCK_SIZE / 64,
          yScale: GameObjects.BLOCK_SIZE / 64,
        },
      },
      restitution: 0.8,
  ...
  static rect(x, y, width, height, material) {
    ...
    let comp = Matter.Composites.stack(
      0,
      0,
      width,
      height,
      ...
      (x, y, cols, rows) => {
        ...
        if (texture == "") {
          ...
          render = {
            sprite: {
              texture: `images/Material Texture/${material}/${texture}.png`,
              xScale: GameObjects.BLOCK_SIZE / 64,
              yScale: GameObjects.BLOCK_SIZE / 64,
            },
          };
  ...
  static mouseConstraint(game) {
    ...
    let scale = {
      x:
        (Generator.WIDTH_RATIO * Generator.RENDER_SCALE) /
        game.renderer.canvas.width,
      y:
        (Generator.HEIGHT_RATIO * Generator.RENDER_SCALE) /
        game.renderer.canvas.height,
    };
  ...
  static slingShot(game, x, y) {
    ...
    Matter.Events.on(game.mouseConstraint, "mouseup", () => {
      let dist = Matter.Vector.magnitude(
        Matter.Vector.sub(constraint.bodyB.position, constraint.pointA)
      );
    ...
    Matter.Events.on(game.engine, "afterUpdate", () => {
      ...
      if (mag > GameObjects.SLING_SHOT_LENGTH) {
        let offset = Matter.Vector.mult(
          diff,
          1 - GameObjects.SLING_SHOT_LENGTH / mag
        );
      ...
      if (constraint.bodyB.launched) {
        ...
        let prevDist = Matter.Vector.magnitude(
        ...
        if (
          ...
          constraint.bodyB = GameObjects.projectile(
            constraint.pointA.x,
            constraint.pointA.y
          );
        }
